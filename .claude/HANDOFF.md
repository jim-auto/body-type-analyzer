...
9. **History rewrite (filter-repo).** One earlier commit briefly contained a `C:\Users\<redacted>\...` path in HANDOFF.md. `git filter-repo --replace-text` stripped it from every reachable commit, and origin was re-added with a `git push --force` publishing the rewrite. Verified: `git log --all -p` now contains zero hits for the user-path pattern.
